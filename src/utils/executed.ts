...
const executeTx = async ({ api, apiReady, network, tx, address, params = {}, errorMessageFallback, onSuccess, onFailed, onBroadcast, setStatus }: Args) => {
			...
			for (const { event } of events) {
				if (event.method === 'ExtrinsicSuccess') {
					...
					if (dispatchError?.isModule) {
						...
						await onFailed(`${dispatchError.type}.${dispatchError.asToken.type}`, txHash);
					} else {
						await onFailed(`${dispatchError.type}` || errorMessageFallback, txHash);
					}
				}
			}
