...
const MemeTokenDetails = ({className, symbol}: Props)=>{
    ...
    const getTokenDetails = async()=>{
        ...
        const {data, error} =await nextApiClientFetch<ICoin>('/api/getSingleTokenDetails', {
           token:symbol
        })
        if(data){
...
useEffect(()=>{
    ...
    return <div className={className}>
        ...
        <Link className="text-primaryButton hover:text-primaryButton flex items-center gap-1" href={'/'}>
        <ArrowLeftOutlined/>
        Back to tokens
        </Link>
        {tokenDetails && <div className="pl-4 w-full flex flex-col gap-4">
            ...
            <div className="border-primaryButton w-1/3 mt-0">
                ...
                <ClockCircleOutlined/>
               <span>{dayjs(tokenDetails?.createdAt).format("D MMM 'YY")}</span>
                </div>
